...
PBJ.on("initialize:after", function(options){
  ...
  $(document).ajaxError( function(e, xhr, options){
    if (xhr.status != 200) {
      ...
      PBJ.errorEnd("ajaxerror", "There has been a server error. <a href='#' onclick='location.reload(true); return false;'>Refresh</a> the site and try again.");
    }
  });
});
...
PBJ.login = function(googleId) {
  ...
  PBJ.vent.trigger("user:loginpending");
  PBJ.infoBegin("loggingin", "Logging in...");
  userSession.save({},{
    success: function(data) {
      PBJ.userSession = data;
      console.log("User logged in:", PBJ.userSession);
      PBJ.vent.trigger("user:loginsuccess", PBJ.userSession);
      PBJ.infoEnd("loggingin", "Login successful.");
      Backbone.history.start();
    },
    error: function(input, response, xhrRequest) {
      PBJ.errorEnd("loggingin", "Login failed.");
      Backbone.history.start();
...
PBJ.errorEnd = function(name, message, parent) {
  opts = {
    name: name,
    parent: parent,
    message: message,
...
PBJ.modalDialogView = null;

PBJ.showModalDialog = function(view, opts) {
  var modal = PBJ.modalDialogView = new PBJ.Views.ModalDialogView();
  modal.setOptions(opts);
  modal.dialogView = view;
  PBJ.dialogRegion.show(modal);
};

PBJ.hideModalDialog = function() {
  PBJ.modalDialogView.closeDialog();
  PBJ.dialogRegion.close();
};
